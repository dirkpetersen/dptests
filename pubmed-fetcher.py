...
# example:
#./pubmed-fetcher.py moin@moin.de 36916030
    ...
    for event, elem in xml.etree.ElementTree.iterparse(response, events=["start", "end"]):
      if event == 'start':
        ...
        if elem.tag == 'MeshHeadingList':
          isMeshList = True
        if isMeshList and elem.tag == 'MeshHeading':
          isMesh = True
        if elem.tag == 'Article':
          isArticle = True
      else:
        ...
        if isMesh and elem.tag == 'DescriptorName':
          medrec.meshs.append('desc:' + elem.text.strip())      
        if isMesh and elem.tag == 'QualifierName':
          medrec.meshs.append('qual:' + elem.text.strip())                    
...
def main():
  ...
  for i in res.pubmed_records:
    ...
    print('\n'.join(x for x in res.pubmed_records[i].references))
    print('Meshs:')
    print('\n'.join(x for x in res.pubmed_records[i].meshs))
